function plotDirectionGraph(filename)
    
    %% 数据准备
    % 检查文件是否存在，如果不存在则使用默认文件名
    if ~exist(filename, 'file')
        filename = 'GA_results.mat';
        if ~exist(filename, 'file')
            error('指定的文件或默认文件(GA_results.mat)未找到。');
        end
    end
    load(filename);
    ...
    theta = thetaStart + (a-1)*thetaStep;
        ...
        for m = 1:Nm
            % 对于所有辐条，都使用第一行的相同r值序列
            r = r_start(m);  % 从第一行提取r值，循环使用
            rm_start(:, (a-1)*Nm + m) = [r*cos(theta), r*sin(theta), 0];
        end
    end
    rm_start = rm_start';
    ...
    for i = 1:length(kx)
            ...
            if kx(i)^2  < 1
                ...
                V_start(i) = sum(exp(1j*k*rm_start*kappa.'));
            else
                % 如果不满足条件，将该位置的值设为0
                V(i) = 0;
                V_optimized(i) = 0;
        ...
        maxVal = max(max(V_start_abs(:)));
        V_start_db = 20*log10(V_start_abs/maxVal);
    
    %% Direction_Graph
    % 定义颜色
    colorOriginal = [47, 127, 193]/255; % 不饱和蓝色
    colorOptimized = [255, 127, 111]/255; % 不饱和红色
    colorstart = [150, 195, 125]/255; % 不饱和绿色
    ...
    % plot(kx, V_start_db, 'color', colorstart, 'LineWidth', 1.5);
